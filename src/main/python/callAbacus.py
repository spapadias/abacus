...
def execute_java(java_file, budget, method, dataset, action, step, seed):
    sep = ":"
    if os.name == 'nt':
        sep = ";"

    # Run java_file at '.' current directory & search at '../../../' for all '*' JAR files
    cmd = 'java -cp ".' + sep + '../../../*" ' + java_file + ' ' + budget + ' ' + method + ' ' + dataset + ' ' + action + ' ' + step + ' ' + seed
...
if __name__ == '__main__':
    ...
    # obtain the memory budget k (or equiv. the maximum possible sample size)
    ...
    # obtain the step (every how many edges to print the butterfly count)
    ...
    javaMain = "BCD/AbacusExperiments"      # Main class in the java project (for Abacus exps)
    ...
    for dataset in datasets:
        for budget in budgets:
            ...
            if not os.path.exists(file2):
                execute_java(javaMain, budget, "perEdgeButterflyCountingSmart", dataset, "Accuracy", step, seed)
            continue